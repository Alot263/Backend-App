    ...
    <aside
        ...
        <div class="navbar-vertical-container">
            <div class="navbar-brand-wrapper justify-content-between">
                ...
                @php($delivery_company_data=Helpers::get_delivery_company_data())
                <a class="navbar-brand" href="{{route('partner.dashboard')}}" aria-label="Front">
                    <img class="navbar-brand-logo initial--36"
                         onerror="this.src='{{asset('public/assets/admin/img/160x160/img2.jpg')}}'"
                         src="{{asset('storage/app/public/delivery_company/'.$delivery_company_data->logo)}}"
                         alt="Logo">
            ...
            <div class="navbar-vertical-content text-capitalize bg--005555" id="navbar-vertical-content">
                ...
                <ul class="navbar-nav navbar-nav-lg nav-tabs">
                    ...
                    @endif
                    @if(Helpers::employee_module_permission_check('order'))
                        <li class="nav-item">
                            <small class="nav-subtitle"
                                   title="{{translate('messages.order')}} {{translate('messages.section')}}">{{translate('messages.order')}} {{translate('messages.section')}}</small>
                        ...
                        <li class="navbar-vertical-aside-has-menu {{Request::is('delivery-company-panel/order*')?'active':''}}">
                            ...
                            <ul class="js-navbar-vertical-aside-submenu nav nav-sub"
                                style="display: {{Request::is('delivery-company-panel/order*')?'block':'none'}}">
                                <li class="nav-item {{Request::is('delivery-company-panel/order/list/all')?'active':''}}">
                                    <a class="nav-link" href="{{route('partner.order.list',['all'])}}"
                                       title="{{translate('messages.all')}} {{translate('messages.orders')}}">
                                        <span class="tio-circle nav-indicator-icon"></span>
                                        <span class="text-truncate sidebar--badge-container">
                                        {{translate('messages.all')}}
                                        <span class="badge badge-soft-info badge-pill ml-1">
                                            {{\App\Models\Order::where('delivery_company_id', Helpers::get_delivery_company_id())
                                                ->where(function($query){
                                                    return $query->whereNotIn('order_status',(config('order_confirmation_model') == 'delivery_company'|| Helpers::get_delivery_company_data()->self_delivery_system)?['failed','canceled', 'refund_requested', 'refunded']:['pending','failed','canceled', 'refund_requested', 'refunded'])
                                ...
                                <li class="nav-item {{Request::is('delivery-company-panel/order/list/item_on_the_way')?'active':''}}">
                                    <a class="nav-link" href="{{route('partner.order.list',['item_on_the_way'])}}"
                                       title="{{translate('messages.items_on_the_way')}}">
                                        <span class="tio-circle nav-indicator-icon"></span>
                                        <span class="text-truncate sidebar--badge-container">
                                        {{translate('messages.item_on_the_way')}}
                                        <span class="badge badge-soft-info badge-pill ml-1">
                                            {{\App\Models\Order::where(['order_status'=>'picked_up', 'delivery_company_id'=>Helpers::get_delivery_company_id()])->StoreOrder()->count()}}
                                        </span>
                                    </span>
                                    </a>
                                </li>
                                <li class="nav-item {{Request::is('delivery-company-panel/order/list/delivered')?'active':''}}">
                                    <a class="nav-link " href="{{route('partner.order.list',['delivered'])}}"
                                       title="{{translate('messages.delivered_orders')}}">
                                        <span class="tio-circle nav-indicator-icon"></span>
                                        <span class="text-truncate sidebar--badge-container">
                                        {{translate('messages.delivered')}}
                                            <span class="badge badge-soft-success badge-pill ml-1">
                                            {{\App\Models\Order::where(['order_status'=>'delivered','delivery_company_id'=>Helpers::get_delivery_company_id()])->StoreOrder()->count()}}
                                        </span>
                                    </span>
                                    </a>
                                </li>
                    ...
                    <!-- DeliveryMan -->
                    @if(Helpers::employee_module_permission_check('deliveryman'))
                        <li class="nav-item">
                            <small class="nav-subtitle"
                    ...
                    <!-- Campaign -->
                    @if(Helpers::employee_module_permission_check('campaign'))
                        <li class="navbar-vertical-aside-has-menu {{Request::is('delivery-company-panel/campaign*')?'active':''}}">
                            ...
                            <ul class="js-navbar-vertical-aside-submenu nav nav-sub"
                                style="display: {{Request::is('delivery-company-panel/campaign*')?'block':'none'}}">
                                <li class="nav-item {{Request::is('delivery-company-panel/campaign/list')?'active':''}}">
                                    <a class="nav-link " href="{{route('partner.campaign.list')}}"
                                ...
                                <li class="nav-item {{Request::is('delivery-company-panel/campaign/item/list')?'active':''}}">
                                    <a class="nav-link " href="{{route('partner.campaign.itemlist')}}"
                    ...
                    <!-- Coupon -->
                    @if (Helpers::employee_module_permission_check('coupon'))
                    ...
                    @if(Helpers::employee_module_permission_check('delivery_company_setup'))
                        <li class="nav-item {{Request::is('delivery-company-panel/business-settings/delivery-company-setup')?'active':''}}">
                            <a class="nav-link " href="{{route('partner.business-settings.delivery-company-setup')}}"
                               title="{{translate('messages.delivery_company')}} {{translate('messages.config')}}"
                            >
                    ...
                    @if(Helpers::employee_module_permission_check('my_delivery_company'))
                    ...
                    @endif
                    @if(Helpers::employee_module_permission_check('bank_info'))
                        <!-- Business Settings -->
                    ...
                    @if(Helpers::employee_module_permission_check('wallet'))
                        <!-- StoreWallet -->
                        <li class="navbar-vertical-aside-has-menu {{Request::is('delivery-company-panel/wallet')?'active':''}}">
                            <a class="js-navbar-vertical-aside-menu-link nav-link"
                               href="{{route('partner.wallet.index')}}" title="{{translate('messages.my_wallet')}}"
                            >
                    ...
                    @endif
                    <!-- End StoreWallet -->
                    @if(Helpers::employee_module_permission_check('reviews'))
                    ...
                    <li class="nav-item">
                        <small class="nav-subtitle"
                               title="{{translate('messages.Report')}} {{translate('messages.section')}}">{{translate('messages.Report')}} {{translate('messages.section')}}</small>
                        <small class="tio-more-horizontal nav-subtitle-replacer"></small>
                    </li>

                    @if(Helpers::employee_module_permission_check('report'))
                        <li class="navbar-vertical-aside-has-menu {{ Request::is('partner/report/expense-report') ? 'active' : '' }}">
                            <a class="nav-link " href="{{ route('partner.report.expense-report') }}"
                    ...
                    <li class="nav-item">
                        <small class="nav-subtitle"
                               title="{{translate('messages.employee')}} {{translate('messages.section')}}">{{translate('messages.employee')}} {{translate('messages.section')}}</small>
                        <small class="tio-more-horizontal nav-subtitle-replacer"></small>
                    </li>

                    @if(Helpers::employee_module_permission_check('custom_role'))
                        <li class="navbar-vertical-aside-has-menu {{Request::is('delivery-company-panel/custom-role*')?'active':''}}">
                            <a class="js-navbar-vertical-aside-menu-link nav-link"
                               href="{{route('partner.custom-role.create')}}"
                    ...
                    @if(Helpers::employee_module_permission_check('employee'))
                        <li class="navbar-vertical-aside-has-menu {{Request::is('delivery-company-panel/employee*')?'active':''}}">
                            ...
                            <ul class="js-navbar-vertical-aside-submenu nav nav-sub"
                                style="display: {{Request::is('delivery-company-panel/employee*')?'block':'none'}}">
                                <li class="nav-item {{Request::is('delivery-company-panel/employee/add-new')?'active':''}}">
                                    <a class="nav-link " href="{{route('partner.employee.add-new')}}"
                                ...
                                <li class="nav-item {{Request::is('delivery-company-panel/employee/list')?'active':''}}">
                                    <a class="nav-link " href="{{route('partner.employee.list')}}"
