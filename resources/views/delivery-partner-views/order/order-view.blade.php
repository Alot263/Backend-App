...
@section('content')
    <?php
    $reasons=\App\Models\OrderCancelReason::where('status', 1)->where('user_type' ,'delivery_company' )->get();
    $tax_included =0;
                            ...
                            <table
                                ...
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        @if ($order->delivery_company->module->module_type == 'food')
                                                ...
                                                <td>
                                                    <div class="media media--sm">
                                                        <a class="avatar avatar-xl mr-3"
                                                            href="{{ route('partner.item.view', $detail->item['id']) }}">
                                                        ...
                                                        <div class="media-body">
                                                            <div>
                                                                ...
                                                                @if ($order->delivery_company && $order->delivery_company->module->module_type == 'food')
                                                                ...
                                                                @else
                                                                    @if (count(json_decode($detail['variation'], true)) > 0)
                                                                        ...
                                                                        @foreach (json_decode($detail['variation'], true)[0] as $key1 => $variation)
                                                                            @if ($key1 != 'stock' || ($order->delivery_company && config('module.' . $order->delivery_company->module->module_type)['stock']))
                            ...
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-right">
                                    ...
                                    <dd class="col-6">
                                        ...
                                        @endif
                                        - {{ \App\CentralLogics\Helpers::format_currency($delivery_company_discount_amount) }}
                                    </dd>
                                    ...
                                    <dd class="col-6">
                                        {{ \App\CentralLogics\Helpers::format_currency($product_price + $del_c + $total_tax_amount + $total_addon_price - $coupon_discount_amount - $delivery_company_discount_amount + $order->dm_tips) }}
                                    </dd>
                                </dl>
                                <!-- End Row -->
                            </div>
                ...
                @if ($order->order_status != 'refund_requested' &&
                    ...
                    <div class="card mb-2">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-4">
                                <div class="row g-1">
                                    <div class="{{ config('canceled_by_delivery_company') ? 'col-6' : 'col-12' }}">
                                        <a class="btn btn--primary w-100 fz--13 px-2 {{ $order['order_status'] == 'pending' ? '' : 'd-none' }}"
                                            onclick="route_alert('{{ route('partner.order.status', ['id' => $order['id'], 'order_status' => 'confirmed']) }}','{{ translate('messages.confirm_this_order') }}')"
                                ...
                                <a class="btn btn--primary w-100 {{ $order['order_status'] == 'confirmed' || $order['order_status'] == 'accepted' ? '' : 'd-none' }}"
                                    onclick="route_alert('{{ route('partner.order.status', ['id' => $order['id'], 'order_status' => 'processing']) }}','{{ translate('messages.proceed_for_processing') }}')"
                                ...
                                <a class="btn btn--primary w-100 {{ $order['order_status'] == 'processing' ? '' : 'd-none' }}"
                                    onclick="route_alert('{{ route('partner.order.status', ['id' => $order['id'], 'order_status' => 'handover']) }}','{{ translate('messages.make_ready_for_handover') }}')"
                                ...
                                <a class="btn btn--primary w-100 {{ $order['order_status'] == 'handover' ? '' : 'd-none' }}"
                                    onclick="order_status_change_alert('{{ route('partner.order.status', ['id' => $order['id'], 'order_status' => 'delivered']) }}','{{ translate('messages.Change status to delivered (payment status will be paid if not)?') }}', {{ $order_delivery_verification ? 'true' : 'false' }})"
    ...
    <div class="modal fade" id="edit-order-amount" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('partner.order.update-order-amount') }}" method="POST" class="row">
                        @csrf
                        ...
                        <div class="form-group col-12">
                            ...
                            <input type="number" class="form-control" name="order_amount" min="0"
                                value="{{ $order['order_amount'] - $order['total_tax_amount'] - $order['delivery_charge'] + $order['delivery_company_discount_amount'] }}" step=".01">
                        </div>
    ...
    <div class="modal fade" id="edit-discount-amount" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('partner.order.update-discount-amount') }}" method="POST" class="row">
                        @csrf
                        <input type="hidden" name="order_id" value="{{ $order->id }}">
                        <div class="form-group col-12">
                            <label for="">{{ translate('messages.discount_amount') }}</label>
                            <input type="number" class="form-control" name="discount_amount" min="0"
                                value="{{ $order['delivery_company_discount_amount'] }}" step=".01">
                        </div>
        ...
        function cancelled_status() {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    location.href = '{!! route('partner.order.status', ['id' => $order['id'],'order_status' => 'canceled']) !!}&reason='+reason,'{{ translate('Change status to canceled ?') }}';
                }
            })
        }
