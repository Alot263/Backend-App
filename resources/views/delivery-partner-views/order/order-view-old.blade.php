            ...
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                        ...
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            <li class="breadcrumb-item">
                                <a class="breadcrumb-link"
                                   href="{{route('partner.order.list',['status'=>'all'])}}">
                    ...
                    <div class="mt-2">
                        <a class="text-body mr-3"
                           href="{{route('partner.order.generate-invoice',[$order['id']])}}">
                        ...
                        <div class="hs-unfold float-right">
                            @if($order['order_status']=='pending')
                            <a class="btn btn-sm btn-primary" onclick="order_status_change_alert('{{route('partner.order.status',['id'=>$order['id'],'order_status'=>'confirmed'])}}','Change status to confirmed ?')" href="javascript:">{{translate('messages.confirm_this_order')}}</a>
                            @if(config('canceled_by_restaurant'))
                            <a class="btn btn-sm btn-danger" onclick="order_status_change_alert('{{route('partner.order.status',['id'=>$order['id'],'order_status'=>'canceled'])}}', '{{translate('messages.order_canceled_confirmation')}}')" href="javascript:">{{translate('messages.cancel_this_order')}}</a>
                            @endif
                            @elseif ($order['order_status']=='confirmed' || $order['order_status']=='accepted')
                            <a class="btn btn-sm btn-primary" onclick="order_status_change_alert('{{route('partner.order.status',['id'=>$order['id'],'order_status'=>'processing'])}}','Change status to processing ?')" href="javascript:">{{translate('messages.Proceed_for_cooking')}}</a>
                            @elseif ($order['order_status']=='processing')
                            <a class="btn btn-sm btn-primary" onclick="order_status_change_alert('{{route('partner.order.status',['id'=>$order['id'],'order_status'=>'handover'])}}','Change status to ready for handover ?')" href="javascript:">{{translate('messages.make_ready_for_handover')}}</a>
                            @elseif ($order['order_status']=='handover' && ($order['order_type']=='take_away' || \App\CentralLogics\Helpers::get_delivery_company_data()->self_delivery_system))
                            <a class="btn btn-sm btn-primary" onclick="order_status_change_alert('{{route('partner.order.status',['id'=>$order['id'],'order_status'=>'delivered'])}}','Change status to delivered (payment status will be paid if not) ?', {{$order_delivery_verification?'true':'false'}})" href="javascript:">{{translate('messages.maek_delivered')}}</a>
                            @endif
                        </div>
                        <!-- End Unfold -->
                    </div>
                ...
                <div class="col-sm-auto">
                    <a class="btn btn-icon btn-sm btn-ghost-secondary rounded-circle mr-1"
                       href="{{route('partner.order.details',[$order['id']-1])}}"
                    ...
                    <a class="btn btn-icon btn-sm btn-ghost-secondary rounded-circle"
                       href="{{route('partner.order.details',[$order['id']+1])}}" data-toggle="tooltip"
                ...
                <div class="card mb-3 mb-lg-5">
                    ...
                    @foreach($order->details as $key=>$detail)
                        @if(isset($detail->item_id) )
                            ...
                            <!-- Media -->
                            <div class="media">
                                <a class="avatar avatar-xl mr-3 cursor-pointer" href="{{route('partner.item.view',$detail->item['id'])}}">
                                    <img class="img-fluid"
                            ...
                            @php($delivery_company_discount_amount += ($detail['discount_on_item']*$detail['quantity']))
                            <!-- End Media -->
                            <hr>
                        ...
                        <div class="row justify-content-md-end mb-3">
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-sm-right">
                                    ...
                                    <dd class="col-sm-6">
                                        - {{\App\CentralLogics\Helpers::format_currency($delivery_company_discount_amount)}}</dd>
                                    ...
                                    <dd class="col-sm-6">{{\App\CentralLogics\Helpers::format_currency($product_price+$del_c+$total_tax_amount+$total_addon_price-$coupon_discount_amount - $delivery_company_discount_amount + $order->dm_tips)}}</dd>
                                </dl>
                                <!-- End Row -->
                            </div>
                        </div>
    ...
    <div id="shipping-address-modal" class="modal fade" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                @if(isset($address))
                    <form action="{{route('partner.order.update-shipping',[$order['delivery_address_id']])}}"
                          method="post">
                        @csrf
                        <div class="modal-body">
                            <div class="row mb-3">
