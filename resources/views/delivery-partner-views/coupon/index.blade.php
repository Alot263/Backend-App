...
@php($delivery_company_data = \App\CentralLogics\Helpers::get_delivery_company_data())
    <div class="content container-fluid">
        ...
        <div class="card mb-3">
            <div class="card-body">
                <form action="{{route('partner.coupon.delivery-company')}}" method="post">
                    @csrf
                    <div class="row">
                        ...
                        <tr>
                            ...
                            {{-- <td>
                                <span class="d-block font-size-sm text-body">
                                    ...
                                    @else
                                    {{translate('messages.delivery_company')}}
                                    @endif
                                </span>
                            </td> --}}
                            {{-- <td>
                            ...
                            <td>
                                <label class="toggle-switch toggle-switch-sm" for="couponCheckbox{{$coupon->id}}">
                                    <input type="checkbox" onclick="location.href='{{route('partner.coupon.status',[$coupon['id'],$coupon->status?0:1])}}'" class="toggle-switch-input" id="couponCheckbox{{$coupon->id}}" {{$coupon->status?'checked':''}}>
                            ...
                            <td>
                                <div class="btn--container justify-content-center">
                                    <a class="btn btn-sm btn--primary btn-outline-primary action-btn" href="{{route('partner.coupon.update',[$coupon['id']])}}"title="{{translate('messages.edit')}} {{translate('messages.coupon')}}"><i class="tio-edit"></i>
                                    </a>
                                    ...
                                    <form action="{{route('partner.coupon.delete',[$coupon['id']])}}"
...
<script>
    ...
    $(document).on('ready', function () {
        ...
        $('#dataSearch').on('submit', function (e) {
            ...
            $.post({
                url: '{{route('partner.coupon.search')}}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
