    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="btn--container align-items-center mb-0">
                ...
                @if (($delivery_company_data->module->module_type == 'food') && $toggle_veg_non_veg)
                <!-- Veg/NonVeg filter -->
                <div>
                ...
                @endif
                <div>
                    <a href="{{route('partner.item.add-new')}}" class="btn btn--primary m-0 pull-right"><i
                ...
                <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                    ...
                    @foreach($items as $key=>$item)
                        <tr>
                            ...
                            <td>
                                <a class="media align-items-center" href="{{route('partner.item.view',[$item['id']])}}">
                                ...
                                <div class="d-flex">
                                    <div class="mx-auto">
                                        <label class="toggle-switch toggle-switch-sm mr-2"  data-toggle="tooltip" data-placement="top" title="{{ translate('messages.Recommend_to_customers') }}" for="recCheckbox{{$item->id}}">
                                            <input type="checkbox" onclick="location.href='{{route('partner.item.recommended',[$item['id'],$item->recommended?0:1])}}'"class="toggle-switch-input" id="recCheckbox{{$item->id}}" {{$item->recommended?'checked':''}}>
                                            <span class="toggle-switch-label">
                                                <span class="toggle-switch-indicator"></span>
                                            </span>
                                        </label>
                            ...
                            <td>
                                <label class="toggle-switch toggle-switch-sm" for="stocksCheckbox{{$item->id}}">
                                    <input type="checkbox" onclick="location.href='{{route('partner.item.status',[$item['id'],$item->status?0:1])}}'"class="toggle-switch-input" id="stocksCheckbox{{$item->id}}" {{$item->status?'checked':''}}>
                            ...
                            <td>
                                <div class="btn--container justify-content-center">
                                    <a class="btn btn-sm btn--primary btn-outline-primary action-btn"
                                        href="{{route('partner.item.edit',[$item['id']])}}" title="{{translate('messages.edit')}} {{translate('messages.item')}}"><i class="tio-edit"></i>
                                    </a>
                                ...
                                <form action="{{route('partner.item.delete',[$item['id']])}}"
...
@push('script_2')
    <script>
        ...
        $('#category').select2({
            ajax: {
                url: '{{route("partner.category.get-all")}}',
