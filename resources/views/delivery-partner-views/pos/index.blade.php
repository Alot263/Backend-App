                        ...
                        <div class="card-body d-flex flex-column" id="items">
                            <div class="row g-3 mb-auto">
                                @foreach ($products as $product)
                                    <div class="order--item-box item-box">
                                        @include('delivery-partner-views.pos._single_product', [
                                            'product' => $product,
                                            'delivery_company' => $delivery_company_data,
                                        ])
                                    </div>
                                @endforeach
                            </div>
                            ...
                            @if (count($products) === 0)
                                <div class="search--no-found">
                                    ...
                                    <p>
                                        {{ translate('messages.no_products_on_delivery_company_pos_search') }}
                                    </p>
                                </div>
                            @endif
                        </div>
                ...
                <div class="order--pos-right">
                    <div class="card">
                        ...
                        <div class="w-100">
                            ...
                            @if ($delivery_company_data->self_delivery_system == 1)
                                <div class="pos--delivery-options">
                                    ...
                                    <div class="pos--delivery-options-info d-flex flex-wrap" id="del-add">
                                        @include('delivery-partner-views.pos._address')
                                    </div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class='w-100' id="cart">
                            @include('delivery-partner-views.pos._cart')
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('partner.pos.customer-delivery_company')}}" method="post" id="product_form">
                        @csrf
                        <div class="row">
...
@push('script_2')
    ...
    <script>
        function initMap() {
            let map = new google.maps.Map(document.getElementById("map"), {
                zoom: 13,
                center: {
                    lat: {{ $delivery_company_data ? $delivery_company_data['latitude'] : '23.757989' }},
                    lng: {{ $delivery_company_data ? $delivery_company_data['longitude'] : '90.360587' }}
                }
            });
            ...
            @if ($delivery_company_data)
                $.get({
                    url: '{{ url('/') }}/admin/zone/get-coordinates/{{ $delivery_company_data->zone_id }}',
                    dataType: 'json',
                            ...
                            geocoder.geocode({
                                ...
                                if (status == google.maps.GeocoderStatus.OK) {
                                    if (results[1]) {
                                        ...
                                        // build request
                                        const origin1 = {
                                            lat: {{ $delivery_company_data['latitude'] }},
                                            lng: {{ $delivery_company_data['longitude'] }}
                                        };
                                        const origin2 = "{{ $delivery_company_data->address }}";
                                        ...
                                        <?php
                                        $module_wise_delivery_charge = $delivery_company_data->zone->modules()->where('modules.id', $delivery_company_data->module_id)->first();
                                        ...
                                        $.get({
                                                url: '{{ route('partner.pos.extra_charge') }}',
                                                dataType: 'json',
                                                data: {
    ...
    <script>
        ...
        function quickView(product_id) {
            $.get({
                url: '{{ route('partner.pos.quick-view') }}',
                dataType: 'json',
                data: {
        ...
        function quickViewCartItem(product_id, item_key) {
            $.get({
                url: '{{ route('partner.pos.quick-view-cart-item') }}',
                dataType: 'json',
                data: {
                    product_id: product_id,
                    item_key: item_key
        ...
        function getVariantPrice() {
            if ($('#add-to-cart-form input[name=quantity]').val() > 0 && checkAddToCartValidity()) {
                ...
                $.ajax({
                    type: "POST",
                    url: '{{ route('partner.pos.variant_price') }}',
        ...
        function addToCart(form_id = 'add-to-cart-form') {
            if (checkAddToCartValidity()) {
                ...
                $.post({
                    url: '{{ route('partner.pos.add-to-cart') }}',
        ...
        function removeFromCart(key) {
            $.post('{{ route('partner.pos.remove-from-cart') }}', {
                _token: '{{ csrf_token() }}',
                key: key
            }, function(data) {
                if (data.errors) {
        ...
        function emptyCart() {
            $.post('{{ route('partner.pos.emptyCart') }}', {
                _token: '{{ csrf_token() }}'
            }, function(data) {
                $('#del-add').empty();
                updateCart();
        ...
        function updateCart() {
            $.post('<?php echo e(route('partner.pos.cart_items')); ?>', {
        ...
        function deliveryAdressStore(form_id = 'delivery_address_delivery_company') {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                }
            ...
            $.post({
                url: '{{ route('partner.pos.add-delivery-info') }}',
                data: $('#' + form_id).serializeArray(),
                beforeSend: function() {
                    $('#loading').show();
                },
        ...
        function payableAmount(form_id = 'payable_delivery_company_amount') {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                }
        ...
        function updateQuantity(e) {
            ...
            if (valueCurrent >= minValue) {
                $.post('{{ route('partner.pos.updateQuantity') }}', {
                    _token: '{{ csrf_token() }}',
                    key: key,
        ...
        $('.js-data-example-ajax').select2({
            ajax: {
                url: '{{ route('partner.pos.customers') }}',
