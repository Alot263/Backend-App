    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                <span>{{translate('messages.update')}} {{translate('messages.delivery_company')}}</span>
            </h1>
        </div>
        <!-- End Page Header -->
        <form action="{{route('admin.delivery-company.update',[$delivery_company['id']])}}" method="post" class="js-validate"
                enctype="multipart/form-data" id="partner_form">
            @csrf
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title m-0 d-flex align-items-center">
                                ...
                                <span>{{translate('delivery_company_information')}}</span>
                            </h4>
                        </div>
                        <div class="card-body">
                            <div class="row g-3 mb-0">
                                <div class="col-lg-4 col-sm-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label" for="name">{{translate('messages.delivery_company')}} {{translate('messages.name')}}</label>
                                        <input type="text" name="name" class="form-control" placeholder="{{translate('messages.delivery_company')}} {{translate('messages.name')}}"
                                                required value="{{$delivery_company->name}}">
                                    </div>
                                </div>
                                <div class="col-lg-4 col-sm-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label" for="address">{{translate('messages.vat/tax')}} (%)</label>
                                        <input type="number" name="tax" class="form-control" placeholder="{{translate('messages.vat/tax')}}" min="0" step=".01" required value="{{$delivery_company->tax}}">
                                    </div>
                                </div>
                                <div class="col-lg-4 col-sm-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label" for="address">{{translate('messages.delivery_company')}} {{translate('messages.address')}}</label>
                                        <textarea  type="text" name="address" class="form-control h--45px" placeholder="{{translate('messages.delivery_company')}} {{translate('messages.address')}}"
                                                required>{{$delivery_company->address}}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="row g-3 my-0">
                                    ...
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="module_id" required class="form-control js-select2-custom"  data-placeholder="{{translate('messages.select')}} {{translate('messages.module')}}" disabled>
                                            ...
                                            @foreach(\App\Models\Module::notParcel()->get() as $module)
                                                <option value="{{$module->id}}" {{$delivery_company->module_id==$module->id?'selected':''}}>{{$module->module_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-8 col-sm-6">
                                    @php
                                        ...
                                        $delivery_time_start = preg_match('([0-9]+[\-][0-9]+\s[min|hours|days])', $delivery_company->delivery_time??'')?explode('-',$delivery_company->delivery_time)[0]:10;
                                        $delivery_time_end = preg_match('([0-9]+[\-][0-9]+\s[min|hours|days])', $delivery_company->delivery_time??'')?explode(' ',explode('-',$delivery_company->delivery_time)[1])[0]:30;
                                        $delivery_time_type = preg_match('([0-9]+[\-][0-9]+\s[min|hours|days])', $delivery_company->delivery_time??'')?explode(' ',explode('-',$delivery_company->delivery_time)[1])[1]:'min';
                                    @endphp
                                    <div class="form-group mb-0">
                            ...
                            <div class="row g-3 my-0">
                                <div class="col-sm-6 col-lg-4">
                                    <div class="d-flex flex-column">
                                        <label class="input-label mb-0">{{translate('messages.delivery_company')}} {{translate('messages.logo')}}<small class="text-danger"> ( {{translate('messages.ratio')}} 1:1 )</small></label>
                                        <center class="py-4 my-auto">
                                            <img class="img--120" id="viewer"
                                                src="{{asset('storage/app/public/delivery_company').'/'.$delivery_company->logo}}" alt="{{$delivery_company->name}}" onerror='this.src="{{asset('public/assets/admin/img/400x400/img2.jpg')}}"'/>
                                        </center>
                                ...
                                <div class="col-sm-6 col-lg-4">
                                    <div class="d-flex flex-column">
                                        ...
                                        <center class="py-4 my-auto">
                                            ...
                                            src="{{asset('storage/app/public/delivery_company/cover/'.$delivery_company->cover_photo)}}" alt="Product thumbnail"/>
                                        </center>
                                        <div class="custom-file">
                                ...
                                <div class="col-lg-4">
                                            ...
                                            @foreach(\App\Models\Zone::active()->get() as $zone)
                                                @if(isset(auth('admin')->user()->zone_id))
                                                    @if(auth('admin')->user()->zone_id == $zone->id)
                                                        <option value="{{$zone->id}}" {{$delivery_company->zone_id == $zone->id? 'selected': ''}}>{{$zone->name}}</option>
                                                    @endif
                                                @else
                                                    <option value="{{$zone->id}}" {{$delivery_company->zone_id == $zone->id? 'selected': ''}}>{{$zone->name}}</option>
                                                @endif
                                    ...
                                    <div class="form-group">
                                        <label class="input-label" for="exampleFormControlInput1">{{translate('messages.latitude')}}<span
                                                class="input-label-secondary" title="{{translate('messages.delivery_company_lat_lng_warning')}}"><img src="{{asset('/public/assets/admin/img/info-circle.svg')}}" alt="{{translate('messages.delivery_company_lat_lng_warning')}}"></span></label>
                                        <input type="text"
                                                name="latitude" class="form-control" id="latitude"
                                                placeholder="{{ translate('messages.Ex:') }} -94.22213" value="{{$delivery_company->latitude}}" readonly>
                                    </div>
                                    <div class="form-group mb-0">
                                        ...
                                        <input type="text"
                                                ...
                                                placeholder="{{ translate('messages.Ex:') }} 103.344322" value="{{$delivery_company->longitude}}" readonly>
                                    </div>
                                </div>
                                <div class="col-lg-8">
                            ...
                            <div class="row g-3">
                                <div class="col-md-4 col-sm-6">
                                    <div class="form-group mb-0">
                                        ...
                                        <input type="text" name="f_name" class="form-control" placeholder="{{translate('messages.first')}} {{translate('messages.name')}}" value="{{$delivery_company->partner->f_name}}"
                                                required>
                                    </div>
                                </div>
                                <div class="col-md-4 col-sm-6">
                                    <div class="form-group mb-0">
                                        ...
                                        value="{{$delivery_company->partner->l_name}}"  required>
                                    </div>
                                </div>
                                <div class="col-md-4 col-sm-6">
                                    <div class="form-group mb-0">
                                        ...
                                        value="{{$delivery_company->phone}}"   required>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-4 col-sm-6">
                                    <div class="form-group mb-0">
                                        ...
                                        <input type="email" name="email" class="form-control" placeholder="{{ translate('messages.Ex:') }} ex@example.com" value="{{$delivery_company->email}}" required>
                                    </div>
                                </div>
                                <div class="col-md-4 col-sm-6">
...
@push('script_2')
    ...
    <script>
        let myLatlng = { lat: {{$delivery_company->latitude}}, lng: {{$delivery_company->longitude}} };
        ...
        function initMap() {
            ...
            new google.maps.Marker({
                position: { lat: {{$delivery_company->latitude}}, lng: {{$delivery_company->longitude}} },
                map,
...
<script>
      $(document).on('ready', function () {
        ...
        get_zone_data({{$delivery_company->zone_id}});
      });
        $("#partner_form").on('keydown', function(e){
            if (e.keyCode === 13) {
                e.preventDefault();
            }
        })
    ...
    <script>
        $('#reset_btn').click(function(){
            $('#viewer').attr('src', "{{asset('storage/app/public/delivery_company').'/'.$delivery_company->logo}}");
            $('#customFileEg1').val(null);
            $('#coverImageViewer').attr('src', "{{asset('storage/app/public/delivery_company/cover/'.$delivery_company->cover_photo)}}");
            $('#coverImageUpload').val(null);
            $('#choice_zones').val("{{$delivery_company->zone_id}}").trigger('change');
            $('#latitude').val("{{$delivery_company->latitude}}");
            $('#longitude').val("{{$delivery_company->longitude}}");
        })
    </script>
@endpush
