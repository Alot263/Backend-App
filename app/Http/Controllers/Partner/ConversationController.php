...
class ConversationController extends Controller
{
    ...
    {
        $partner = Helpers::get_partner_data();
        $partner = UserInfo::where('partner_id',$partner->id)->first();
        if($partner){
            $conversations = Conversation::with(['sender','receiver', 'last_message'])->WhereUser($partner->id);
        ...
        if ($request->ajax()) {
            ...
            $view = view('delivery-partner-views.messages.data',compact('conversations'))->render();
            return response()->json(['html'=>$view]);
        }

        return view('delivery-partner-views.messages.index', compact('conversations'));
    }

    public function view($conversation_id,$user_id): JsonResponse
    {
        ...
        return response()->json([
            'view' => view('delivery-partner-views.messages.partials._conversations', compact('convs', 'user', 'receiver','sender','user_type','partner'))->render()
        ]);
    }

    public function delivery_company(Request $request, $user_id, $user_type)
    {
        ...
        return response()->json([
            'view' => view('delivery-partner-views.messages.partials._conversations', compact('convs', 'user', 'receiver','user_type','partner'))->render()
        ]);
    }
}
