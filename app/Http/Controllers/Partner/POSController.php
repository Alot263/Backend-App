...
class POSController extends Controller
{
    public function index(Request $request)
    {
        ...
        $categories = Category::active()->module(Helpers::get_delivery_company_data()->module_id)->get();
        $keyword = $request->query('keyword', false);
        $delivery_company = DeliveryCompany::find(Helpers::get_delivery_company_data()->module_id);
        $key = explode(' ', $keyword);
        ...
        ->latest()->paginate(10);
        return view('delivery-partner-views.pos.index', compact('categories', 'products','delivery_company','category', 'keyword'));
    }
    ...
    public function quick_view(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => 1,
            'view' => view('delivery-partner-views.pos._quick-view-data', compact('product'))->render(),
        ]);
    }

    public function quick_view_card_item(Request $request): JsonResponse
    {
        $product = Item::findOrFail($request->product_id);
        ...
        return response()->json([
            'success' => 1,
            'view' => view('delivery-partner-views.pos._quick-view-cart-item', compact('product', 'cart_item', 'item_key'))->render(),
        ]);
    }
    ...
    public function variant_price(Request $request)
    {
        ...
        if($product->module->module_type == 'food' && $product->food_variations){
            ...
            if ($request->variations && count($product_variations)) {
                ...
                $price= $price_total - Helpers::product_discount_calculate($product, $price_total, $product->delivery_company);
            } else {
                $price = $product->price - Helpers::product_discount_calculate($product, $product->price, $product->delivery_company);
            }
        }else{
            ...
            if ($str != null) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    if (json_decode($product->variations)[$i]->type == $str) {
                        $price = json_decode($product->variations)[$i]->price - Helpers::product_discount_calculate($product, json_decode($product->variations)[$i]->price,Helpers::get_delivery_company_data());
                    }
                }
            } else {
                $price = $product->price - Helpers::product_discount_calculate($product, $product->price,Helpers::get_delivery_company_data());
            }
        }
    ...
    public function addDeliveryInfo(Request $request)
    {
        ...
        return response()->json([
            'data' => $address,
            'view' => view('delivery-partner-views.pos._address', compact('address'))->render(),
        ]);
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $data['discount'] = Helpers::product_discount_calculate($product, $price,Helpers::get_delivery_company_data());
    ...
    {
        return view('delivery-partner-views.pos._cart');
    }
    ...
    public function place_order(Request $request)
    {
        ...
        $order->delivery_company_id = $delivery_company->id;
        $order->module_id = Helpers::get_delivery_company_data()->module_id;
        ...
        foreach ($cart as $c) {
            if(is_array($c))
            {
                $product = Item::withoutGlobalScope(DeliveryCompanyScope::class)->find($c['id']);
                if ($product) {
                    if($product->module->module_type == 'food'){
                        ...
                        $price = $c['price'];
                        $product->tax = $product->delivery_company->tax;
                        $product = Helpers::product_data_formatting($product);
                        $addon_data = Helpers::calculate_addon_price(\App\Models\AddOn::withOutGlobalScope(DeliveryCompanyScope::class)->whereIn('id', $c['add_ons'])->get(), $c['add_on_qtys']);
                        $or_d = [
                            'item_id' => $c['id'],
                            ...
                            'discount_on_item' => Helpers::product_discount_calculate($product, $price, $product->delivery_company),
                        ...
                        $delivery_company_discount_amount += $or_d['discount_on_item'] * $or_d['quantity'];
                        $order_details[] = $or_d;
                    }else{
                        ...
                        $price = $c['price'];
                        $product->tax = $delivery_company->tax;
                        $product = Helpers::product_data_formatting($product);
                        ...
                        $or_d = [
                            ...
                            'price' => $price,
                            'tax_amount' => Helpers::tax_calculate($product, $price),
                            'discount_on_item' => Helpers::product_discount_calculate($product, $price, $delivery_company),
                            'discount_type' => 'discount_on_product',
        ...
        {
            $delivery_company_discount_amount += $cart['discount_type']=='percent'&&$cart['discount']>0?((($product_price + $total_addon_price - $delivery_company_discount_amount) * $cart['discount'])/100):$cart['discount'];
        }

        $total_price = $product_price + $total_addon_price - $delivery_company_discount_amount;
        $tax = isset($cart['tax'])?$cart['tax']:$delivery_company->tax;
        ...
        try {
            ...
            //     $order->tax_status = 'included';
            // }
            $order->delivery_company_discount_amount= $delivery_company_discount_amount;
    ...
    public function order_list()
    {
        ...
        ->where('delivery_company_id',\App\CentralLogics\Helpers::get_delivery_company_id())
        ->latest()
        ->paginate(config('default_pagination'));

        return view('delivery-partner-views.pos.order.list', compact('orders'));
    }

    public function search(Request $request){
        $key = explode(' ', $request['search']);
        $orders=Order::where(['delivery_company_id'=>Helpers::get_delivery_company_id()])->where(function ($q) use ($key) {
            foreach ($key as $value) {
        ...
        return response()->json([
            'view'=>view('delivery-partner-views.pos.order.partials._table',compact('orders'))->render()
        ]);
    }

    public function order_details($id)
    {
        $order = Order::with('details')->where(['id' => $id, 'delivery_company_id' => Helpers::get_delivery_company_id()])->first();
        if (isset($order)) {
            return view('delivery-partner-views.pos.order.order-view', compact('order'));
        } else {
    ...
    public function generate_invoice($id)
    {
        ...
        return response()->json([
            'success' => 1,
            'view' => view('delivery-partner-views.pos.order.invoice', compact('order'))->render(),
        ]);
    }

    public function customer_delivery_company(Request $request)
    {
        $request->validate([
            'f_name' => 'required',
            'l_name' => 'required',
