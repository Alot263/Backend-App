...
class OrderController extends Controller
{
    ...
    {
        Order::where(['checked' => 0])->where('delivery_company_id',Helpers::get_delivery_company_id())->update(['checked' => 1]);
        ...
        ->when($status == 'all', function($query){
            return $query->where(function($query){
                $query->whereNotIn('order_status',(config('order_confirmation_model') == 'delivery_company'|| Helpers::get_delivery_company_data()->self_delivery_system)?['failed','canceled', 'refund_requested', 'refunded']:['pending','failed','canceled', 'refund_requested', 'refunded'])
        ...
        return view('delivery-partner-views.order.list', compact('orders', 'status'));
    }
    ...
    public function search(Request $request){
        ...
        return response()->json([
            'view'=>view('delivery-partner-views.order.partials._table',compact('orders'))->render()
        ]);
    }
    ...
    public function details(Request $request,$id)
    {
        $order = Order::with(['details', 'customer'=>function($query){
            ...
            return $query->withCount('orders');
        }])->where(['id' => $id, 'delivery_company_id' => Helpers::get_delivery_company_id()])->first();
        if (isset($order)) {
            return view('delivery-partner-views.order.order-view', compact('order'));
        } else {
    ...
    public function status(Request $request): RedirectResponse
    {
        ...
        $order = Order::where(['id' => $request->id, 'delivery_company_id' => Helpers::get_delivery_company_id()])->first();
        ...
        if($request['order_status']=='canceled' && !config('canceled_by_delivery_company'))
        {
        ...
        if($request['order_status']=='delivered' && $order->order_type != 'take_away' && !Helpers::get_delivery_company_data()->self_delivery_system)
        {
        ...
        {
            if(!Helpers::get_delivery_company_data()->self_delivery_system && config('order_confirmation_model') == 'deliveryman' && $order->order_type != 'take_away')
            {
    ...
    {
        $order = Order::where(['id' => $id, 'delivery_company_id' => Helpers::get_delivery_company_id()])->first();
        return view('delivery-partner-views.order.invoice', compact('order'));
    }

    public function add_payment_ref_code(Request $request, $id)
    {
        Order::where(['id' => $id, 'delivery_company_id' => Helpers::get_delivery_company_id()])->update([
