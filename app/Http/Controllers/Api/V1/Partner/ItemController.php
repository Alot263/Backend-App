...
class ItemController extends Controller
{
    ...
    public function delivery_company(Request $request): JsonResponse
    {
        ...
        $item->delivery_company_id = $request['partner']->delivery_companies[0]->id;
        $item->veg = $request->veg;
        $item->module_id = $request['partner']->delivery_companies[0]->module_id;
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        ->with(['rating'])
        ->where('delivery_company_id', $request['partner']->delivery_companies[0]->id)
        ->when($request->category_id, function($query)use($request){
            $query->whereHas('category',function($q)use($request){
                return $q->whereId($request->category_id)->orWhere('parent_id', $request->category_id);
            });
        })
        ->when($request->delivery_company_id, function($query) use($request){
    ...
    {
        $id = $request['partner']->delivery_companies[0]->id;;

        $reviews = Review::with(['customer', 'item'])
        ->whereHas('item', function($query)use($id){
            return $query->where('delivery_company_id', $id);
        })
        ->latest()->get();
