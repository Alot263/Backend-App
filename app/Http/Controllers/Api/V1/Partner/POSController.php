...
class POSController extends Controller
{
    public function place_order(Request $request)
    {
        ...
        {
            $delivery_company_discount_amount += $request['discount_type']=='percent'&&$request['discount']>0?((($product_price + $total_addon_price) * $request['discount'])/100):$request['discount'];
        }
        $delivery_company_discount_amount = round($delivery_company_discount_amount ,2);
        $total_price = $product_price + $total_addon_price - $delivery_company_discount_amount;
        $tax = isset($request['tax'])?$request['tax']:$delivery_company->tax;
    ...
    public function generate_invoice(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'order_id' => 'required'
        ]);
        ...
        {
            return response()->json([
                'view' => view('partner-views.pos.order.invoice', compact('order'))->render(),
            ]);
        }
