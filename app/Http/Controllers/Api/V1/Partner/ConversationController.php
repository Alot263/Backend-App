...
{
    public function messages_delivery_company(Request $request)
    {
        ...
        $partner = Partner::find($request->partner->id);
        $sender = UserInfo::where('partner_id', $partner->id)->first();
        if(!$sender){
            $sender = new UserInfo();
            $sender->partner_id = $partner->id;
            $sender->f_name = $partner->delivery_companies[0]->name;
            $sender->l_name = '';
            $sender->phone = $partner->phone;
            $sender->email = $partner->email;
            $sender->image = $partner->delivery_companies[0]->logo;
            $sender->save();
        }
        ...
        if(!$conversation){
            ...
            $conversation->sender_type = 'partner';
        ...
        if($conv->sender_type == 'customer' && $conversation->sender){
            ...
            $order = Order::where('delivery_company_id',$partner->delivery_companies[0]->id)->where('user_id', $user->id)->whereIn('order_status', ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
            ...
            $order = Order::where('delivery_company_id',$partner->delivery_companies[0]->id)->where('delivery_man_id', $user2->id)->whereIn('order_status', ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
    ...
    public function search_conversations(Request $request): JsonResponse
    {
    ...
    public function messages(Request $request): JsonResponse
    {
        ...
        $vnd = Partner::find($request->partner->id);
        $partner = UserInfo::where('partner_id', $vnd->id)->first();
        if(!$partner){
            $partner = new UserInfo();
            $partner->partner_id = $vnd->id;
            $partner->f_name = $vnd->delivery_companies[0]->name;
            $partner->l_name = '';
            $partner->phone = $vnd->phone;
            $partner->email = $vnd->email;
            $partner->image = $vnd->delivery_companies[0]->logo;
            $partner->save();
        }
        ...
        if($request->conversation_id){
            ...
            $conversation = Conversation::with(['sender','receiver','last_message'])->WhereConversation($partner->id,$dm->id)->first();
            ...
            $conversation = Conversation::with(['sender','receiver','last_message'])->WhereConversation($partner->id,$user->id)->first();
        }
        ...
        if($conversation){
            ...
            if($conversation->sender_type == 'customer' && $conversation->sender){
                ...
                $order = Order::where('delivery_company_id',$vnd->delivery_companies[0]->id)->where('user_id', $user->id)->whereIn('order_status', ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
                ...
                $order = Order::where('delivery_company_id',$vnd->delivery_companies[0]->id)->where('delivery_man_id', $user2->id)->whereIn('order_status', ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
            ...
            if($lastmessage && $lastmessage->sender_id != $partner->id ) {
                $conversation->unread_message_count = 0;
                $conversation->save();
            }
            Message::where(['conversation_id' => $conversation->id])->where('sender_id','!=', $partner->id)->update(['is_seen' => 1]);
            $messages = Message::where(['conversation_id' => $conversation->id])->latest()->paginate($limit, ['*'], 'page', $offset);
        }else{
            $messages =[];
            $order=0;
