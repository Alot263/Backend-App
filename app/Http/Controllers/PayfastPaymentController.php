...
class PayfastPaymentController extends Controller
{
	...
	public function token($config, $base_url){
		...
		curl_setopt($ch, CURLOPT_USERPWD, $config['payfast_merchant_id'] . ':' . $config['payfast_merchant_key']. ':' . $config['payfast_passphrase']. ':' . $config['payfast_baseurl']);
	...
	public function payment(Request $request)
	{
		...
		if($config){
			$base_url = ($config['mode'] == 'test') ? 'https://sandbox.payfast.co.za/eng/process' : 'https://api-m.payfast.com';
		}
		...
		if ( isset($accessToken['access_token'])) {
			...
			$payment_data['application_context'] = [
				'return_url' => route('payfast.callback',['payment_id' => $order->id]),
				'cancel_url' => route('payfast.cancel',['payment_id' => $order->id])
			];
			$ch = curl_init();
			...
			$headers[] = "Payfast-Request-Id:".Str::uuid();
			curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
	...
	public function callback(Request $request)
	{
		...
		if($response->status === 'COMPLETED'){
			...
			try {
				...
				info(['payfast_issue' => $e->getMessage()]);
			}
