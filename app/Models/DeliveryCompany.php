...
class DeliveryCompany extends Model
{
    ...
    {
        return $this->hasMany(DeliveryCompanySchedule::class)->orderBy('opening_time');
    }
    ...
    {
        $query->selectRaw('*, IF(((select count(*) from `delivery_company_schedule` where `delivery_companies`.`id` = `delivery_company_schedule`.`company_id` and `delivery_company_schedule`.`day` = '.now()->dayOfWeek.' and `delivery_company_schedule`.`opening_time` < "'.now()->format('H:i:s').'" and `delivery_company_schedule`.`closing_time` >"'.now()->format('H:i:s').'") > 0), true, false) as open,ST_Distance_Sphere(point(longitude, latitude),point('.$longitude.', '.$latitude.')) as distance');
    }
    ...
    {
        parent::boot();
        static::created(function ($delivery_company) {
            $delivery_company->slug = $delivery_company->generateSlug($delivery_company->name);
